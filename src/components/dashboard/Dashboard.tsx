...
function Dashboard() {
  return (
    <div className="shadow-2xl my-[12vh] p-8">
      <p className="text-center uppercase text-xl xs:text-3xl text-blue-300 py-8 shadow-2xl rounded mt-2 mb-10"><span className="hidden md:inline">"Discover Zanzibar: </span>Wonders Tour Package<span className="hidden md:inline">"</span></p>
      {
          ...
          return (
            <div key={item.id} className={`flex flex-col md:grid md:grid-cols-2 md:gap-[5vw] mb-[15vh]`}>
              <div className={`flex flex-col ${i % 2 === 0 ? 'md:order-2' : 'md:order-1'}`}>
                ...
                <p className="px-2 text-start md:py-8 shadow font-montserrat text-gray-500 dark:text-white tracking-wide mb-6">{item.text}</p>
            </div>
